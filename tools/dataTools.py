...
def get_colormap(categories: np.array, cmap: str = 'plasma_r',  truncate: bool = True):
    ...
    if truncate:
        colors = [cm.get_cmap(cmap)(i)
                  for i in np.linspace(0.1, 0.9, len(color_labels))]
    else:
...
def get_moveset(repertoire: str):
    ...
    elif repertoire in Constants.AMPSA1:
        return Constants.AMPA1_MOVESET
    elif repertoire in Constants.AMPSB1:
        return Constants.AMPB1_MOVESET
    else:
...
def get_repertoire_data(dataset: str, repertoire: str, seed: int):
    ...
    datname = Constants.PROJ_DIR + Constants.DATA_FOLDER + \
        str(seed) + '/' + repertoire + dataset
...
def get_outdir(seed: int, sim_set: str, repertoire: str, perturbation: str = None, pert_param: float = None, pert_set: str = None, config_num: int = None):
    """ 
    ...
    """
    info = [sim_set, str(seed), repertoire, str(
        config_num) if config_num else None]
    if perturbation:
        info.extend([perturbation, str(pert_set)
                    if pert_set else None, str(pert_param)])

    outdir = Constants.PROJ_DIR + Constants.RESULTS_FOLDER + \
...
def graph_position(positions, ax, task_info, cmap=None, graph_all=False, graph_ntrials=15, is_reaches_moveset=True, graph_movement=True, **plot_kwargs):
    """
    ...
    graph_ntrials: number of trials to graph
    is_reaches_moveset: whether to graph reaches or sinewaves
    graph_movement: whether to graph movement or x and y positions separately
    ...
    if cmap is not None:
        ...
        if len(np.unique(task_info)) == 1 or (not is_reaches_moveset):
    ...
    if graph_all:
        ...
        for target in np.unique(task_info):
            target_idx = np.random.choice(
                np.where(task_info == target)[0], graph_ntrials)
    ...
    # if 2D output (reaches)
    if is_reaches_moveset:
        ...
        ax.set_ylim([-y_pos, 1])
        ax.set_axis_off()
    # if 1D output (sinewaves)
    else:
        if positions.shape[2] == 1:
            for i in range(positions.shape[0]):
                ax.plot(positions[i, :, j], '.', c=colormap[task_info[i]],
                        linestyle='-', linewidth=1, marker=None, **plot_kwargs)
            ax.set_ylim([-4.2, 4.2])
        else:
            #TODO: better define go/cue onsets
            # start=0 #130
            # stop=positions.shape[1] #330
            start = 130
            stop = 330
            if graph_movement:
                for i in range(positions.shape[0]):
                    ax.plot(positions[i, start:stop+1, 1], positions[i, start:stop+1, 0], '.', c=colormap[task_info[i]],
                            linestyle='-', linewidth=0.5, marker=None, **plot_kwargs)
                # ax.set_ylim([-7.5, 7.5])
                ax.set_aspect(1)
            else:
                for i in range(positions.shape[0]):
                    ax.plot(positions[i, start:stop+1, 1], '.', c=colormap[task_info[i]],
                            linestyle='-', linewidth=1, marker=None, **plot_kwargs)

                    ax.plot(positions[i, start:stop+1, 0] - 9, '.', c=colormap[task_info[i]],
                            linestyle='-', linewidth=1, marker=None, **plot_kwargs)

                    # ax.plot(positions[i, :, 0], positions[i, :, 1], '.', c=colormap[task_info[i]], linestyle='-', linewidth=1, marker=None, **plot_kwargs)
...
def graph_repertoire(dataset: str, repertoire: str, seed: int = Constants.SEED_EX, ax: plt.Axes = None, cmap: str = None, graph_all: bool = False, graph_ntrials: int = 15, graph_movement: bool = True, **plot_kwargs):
    """ 
    ...
    target_param = data['stimulus_param']
    target = data['target']
    task_info = np.array(target_param)

    is_reaches_moveset = not (('amp' in moveset) or ('freq' in moveset))

    graph_position(target, ax, task_info, cmap=cmap,
                   graph_all=graph_all, graph_ntrials=graph_ntrials,
                   is_reaches_moveset=is_reaches_moveset, graph_movement=graph_movement,
                   **plot_kwargs)

    return ax


def graph_output(seed: int, sim_set: str, repertoire: str, perturbation: str = None, pert_param: float = 0, pert_set: str = None, config_num: int = None, ax: plt.Axes = None, epoch: int = None, datafile: str = None, noise: float = None, graph_all: bool = False, graph_ntrials: int = 15, graph_movement: bool = True, **plot_kwargs):
    """ 
    ...
    outdir = get_outdir(seed, sim_set, repertoire,
                        perturbation, pert_param, pert_set, config_num)
    datadir, _, output, _ = test_model(
        outdir, epoch, datafile=datafile, noise=noise)
    ...
    is_reaches_moveset = not (('amp' in sim_set) or ('freq' in sim_set))

    graph_position(output, ax, task_info, graph_all=graph_all, graph_ntrials=graph_ntrials,
                   is_reaches_moveset=is_reaches_moveset, graph_movement=graph_movement, **plot_kwargs)
    return ax


def graph_outputs_all(seed: int, sim_set: str, repertoires: List[str], perturbation: str = None, pert_param: float = 0, pert_set: str = None, config_num: int = None, epoch: int = None, datafile: str = None, noise: float = None, graph_all: bool = False, learning_datafile: bool = False, graph_ntrials: int = 15, graph_movement: bool = True):
    """ 
    ...
    """
    if (Constants.AMPA1_MOVESET in sim_set) or (Constants.AMPB1_MOVESET in sim_set):
        fig, axs = plt.subplots(nrows=(len(repertoires)))
    else:
        fig, axs = plt.subplots(ncols=(len(repertoires)),
                                figsize=(2*len(repertoires), 2))
    for i, repertoire in enumerate(repertoires):
        if learning_datafile:
            datafile = repertoire + Constants.SIM_SET_DATA[sim_set]
        graph_output(seed, sim_set, repertoire, perturbation, pert_param, pert_set=pert_set, config_num=config_num,
                     ax=axs[i], epoch=epoch, datafile=datafile, noise=noise, graph_all=graph_all, graph_ntrials=graph_ntrials, graph_movement=graph_movement)

    # save plot
    params = '_'.join(filter(None, [sim_set, str(seed), perturbation, pert_set,
                                    str(pert_param) if pert_param else None, str(
                                        config_num) if config_num else None, datafile,
...
def get_loss(seed: int, sim_set: str, repertoire: str, perturbation: str = None, pert_param: float = None, pert_set: str = 'None', config_num: int = None, smooth: bool = False, smoothing_interval: int = 5, measure: str = 'loss'):
    """ 
    ...
    if smooth:
        ...
        loss = uniform_filter1d(loss, size=smoothing_interval, origin=math.floor(
            smoothing_interval/2), mode='nearest')
    return loss


def get_loss_df(seeds: List[int], sim_set: str, repertoires: List[str], perturbation: str = None, pert_param: float = None, pert_set: str = 'None', config_num: int = None, smooth: bool = False, smoothing_interval: int = 5,  measure: str = 'loss'):
    """ 
    ...
    for i in range(len(repertoires)):
        ...
        for seed in seeds:
            loss = get_loss(seed, sim_set, repertoire, perturbation, pert_param, pert_set, config_num,
                            smooth, smoothing_interval, measure)
            repertoire_loss_df = pd.DataFrame({"loss": loss})
            repertoire_loss_df['repertoire'] = repertoire
            repertoire_loss_df['ntasks'] = repertoire.count(
                '_') - 1  # TODO: fix this temp solution
...
def graph_loss(seeds: List[int], sim_set: str, repertoires: List[str], perturbation: str = None, pert_param: float = None, pert_set: str = None, config_num: int = None, smooth: bool = False, smoothing_interval: int = 5, rel_loss: bool = False, measure: str = 'loss', ax: plt.Axes = None, xlim=None, colors=None):
    """ 
    ...
    # get loss dataframe
    loss_df = get_loss_df(seeds, sim_set, repertoires, perturbation, pert_param, pert_set, config_num,
                          smooth, smoothing_interval, measure)
    ...
    colors = colors or [cm.get_cmap('plasma_r')(i)
                        for i in np.linspace(0.1, 0.9, len(repertoires))]
    g = sns.lineplot(ax=ax, data=loss_df, x="epoch", y=loss,
                     hue="repertoire", palette=colors)
    g.set(xlabel='Training trial', ylabel=(
        'Relative loss' if rel_loss else 'Loss'))
    g.get_legend().remove()
    ...
    params = '_'.join(filter(None, [sim_set, str(seed) if seed else None, perturbation, pert_set,
                                    str(pert_param) if pert_param else None, str(config_num) if config_num else None, "smooth" if smooth else None, str(xlim) if xlim else None]))
    return g, params


def graph_loss_broken_axis(seeds: List[int], sim_set: str, repertoires: List[str], perturbation: str = None, pert_param: float = None, pert_set: str = None, config_num: int = None, smooth: bool = False, rel_loss: bool = False, measure: str = 'loss', fig_ax=None):
    """
    ...
    g, _ = graph_loss(seeds, sim_set, repertoires, perturbation, pert_param, pert_set, config_num,
    ...
    params = '_'.join(filter(None, [sim_set, perturbation, pert_set,
                                    str(pert_param) if pert_param else None, str(config_num) if config_num else None, "smooth" if smooth else None]))
    return ax_left, ax_right, params


def get_testing_MSE(seed: int, sim_set: str, repertoire: str,  perturbation: str = None, pert_param: float = None, pert_set: str = None, config_num: int = None, noise: float = None, datafile: str = None, epoch: int = None, rotate_angle: float = 0.0):
    """
    ...
    outdir = get_outdir(seed, sim_set, repertoire,
                        perturbation, pert_param, pert_set, config_num)
    datadir, _, output, _ = test_model(
        outdir, noise=noise, datafile=datafile, epoch=epoch)
...
def get_testing_MSE_by_component(seed: int, sim_set: str, repertoire: str,  perturbation: str = None, pert_param: float = None, pert_set: str = None, config_num: int = None, noise: float = None, datafile: str = None, epoch: int = None, rotate_angle: float = 0.0):
    """
    Get testing MSE for a given model.
...
def get_testing_MSE_by_target(seed: int, sim_set: str, repertoire: str, perturbation: str = None, pert_param: float = None, pert_set: str = None, config_num: int = None, noise: float = None, datafile: str = None, epoch: int = None, rotate_angle: float = 0.0):
    """
...
def get_testing_MSE_df(seeds: List[int], sim_set: str, repertoires: List[str],
                       perturbation: str = None, pert_param: float = None, pert_set: str = None, config_num: int = None, noise: float = None, datafile: str = None, learning_datafile: bool = False, by_target=False, epoch=None, rotate_angle=0.0):
    """
...
def model_to_pyaldata(sim_set: str, seed: int, repertoire: str, perturbation: str = None, pert_param: float = 0, pert_set: str = None, config_num: int = None, epoch: int = None, datafile: str = None, noise: float = None, calculate_kinematics: bool = False):
    """
    ...
    outdir = get_outdir(seed, sim_set, repertoire,
                        perturbation, pert_param, pert_set, config_num)
    datadir, _, output, activity1 = test_model(
        outdir, epoch, datafile, noise=noise)
    ...
    if calculate_kinematics:
        ...
        for trial in range(ntrials):
            ...
            for tstep in range(go_step, tsteps):
                # calculate velocity
                vels[trial, tstep, 0] = (
                    pos[trial, tstep, 0]-pos[trial, tstep-1, 0])/dt
                vels[trial, tstep, 1] = (
                    pos[trial, tstep, 1]-pos[trial, tstep-1, 1])/dt
...
def preprocess_data(pyal_df: pd.DataFrame, epoch_fun: Callable = None, subtract_mean: bool = False, smooth_signals=True):
    """
    ...
    if 'M1_rates' in pyal_df.columns:
        pyal_df = pyal.merge_signals(
            pyal_df, ["M1_rates", "PMd_rates"], "MCx_rates")

        if smooth_signals:
            pyal_df = pyal.smooth_signals(
                pyal_df, ["M1_rates", "PMd_rates", "MCx_rates"])
    else:
        if smooth_signals:
...
def get_pyaldata(sim_set: str, seed: int, repertoire: str, perturbation: str = None, pert_param: float = None, pert_set: str = None, config_num: int = None, epoch_fun: Callable = None, epoch: int = None, datafile: str = None, subtract_mean: bool = False, noise: float = None, calculate_kinematics: bool = False, smooth_signals: bool = True):
    """
    ...
    pyal_df = model_to_pyaldata(sim_set, seed, repertoire, perturbation, pert_param, pert_set, config_num,
                                epoch=epoch, datafile=datafile, noise=noise, calculate_kinematics=calculate_kinematics)
    pyal_df = preprocess_data(
        pyal_df, epoch_fun=epoch_fun, subtract_mean=subtract_mean, smooth_signals=smooth_signals)
...
def perform_pca(pyal_df: pd.DataFrame, pca_dims: int):
    ...
    if 'M1_rates' in pyal_df.columns:
        ...
        pyal_df = pyal.dim_reduce(pyal_df, PCA(
            pca_dims), "PMd_rates", "PMd_pca")
...
def compare_measure(measures_df: pd.DataFrame, measure: str, reps: List[str], norm_factor: str = None, estimator: Callable = np.median, stats_alternative: str = 'greater', ci: int = 95, seeds=Constants.SEEDS, ax: plt.Axes = None):
    """
    ...
    if norm_factor is not None:
        measures_df[measure+'_norm'] = measures_df[measure] * \
            measures_df[norm_factor]
    ...
    g = pointplot_across_seeds(
        df, reps, seeds, measure, estimator=estimator, ci=ci, ax=ax)

    # stats
    measures_df[measure + '_est'] = measures_df.apply(
        lambda row: estimator(row[measure]), axis=1)
    for i, rep1 in enumerate(reps[:-1]):
        a = measures_df[(measures_df.repertoire == reps[i])
                        ][measure + '_est'].values
        b = measures_df[(measures_df.repertoire == reps[i+1])
                        ][measure + '_est'].values

        # check normality: this is not normal
        # print(rep1, scipy.stats.shapiro(a), a)
        _, pnorm = scipy.stats.wilcoxon(a, b, alternative=stats_alternative)
        text = get_signif_annot(pnorm)
        print(pnorm)

        g.annotate(text, xy=((i+1)*0.25, 0.9), xytext=((i+1)*0.25, 0.95), xycoords='axes fraction', fontsize='x-large',
                   ha='center', va='bottom', arrowprops=dict(arrowstyle='-[, widthB=1.3, lengthB=0.4', lw=1))
    return g
...
def pointplot_across_seeds(data: pd.DataFrame, repertoires: List[str], seeds: List[int], y: str, estimator=np.median, ax: plt.Axes = None, ci: int = 95):
    ...
    g = sns.pointplot(x="repertoire", y=y, data=df, markers="", estimator=estimator,
                      join=True, ci=None, hue='seed', palette=line_colors, ax=ax, order=repertoires)
    ...
    for seed in seeds:
        ...
        g = sns.pointplot(x="repertoire", y=y, data=df_, estimator=estimator,
                          ...
                          ci=ci, ax=ax, order=repertoires)

    g.set_xticklabels([abr_dict[s] if s in abr_dict.keys()
                      else s for s in repertoires])
...
def plot_3d(df: pd.DataFrame, signal: str, dims: List[int] = [1, 2, 3], rel_start: int = 0, elev: int = None, azim: int = None, ax: plt.Axes = None, alpha: float = 1, linestyle: str = 'solid', is_reaches_moveset: bool = True):
    """
    ...
    dims: dimensions to plot, list of 3 ints
    ...
    alpha: opacity
    linestyle: linestyle
    is_reaches_moveset: whether to use colors for reaches
    """

    if len(np.unique(df.target_param.values)) == 1 or (not is_reaches_moveset):
        colormap = get_colormap(df.target_param.values,
                                cmap=Constants.MOVEMENT_CMAP)
    else:
...
def pointplot_across_repertoires(data: pd.DataFrame, repertoires: List[str], y: str, color='grey', linestyle: str = 'solid', ax: plt.Axes = None, abr_labels: bool = True, label=None):
    """
    ...
    linestyle: linestyle for the connecting line
    ax: axis to plot on
    abr_labels: whether to use abbreviated labels for the repertoires
    """

    df = data[data.repertoire.isin(repertoires)]
    # TODO: add repertoire colors dict in constants
    colors = Constants.REPERTOIRE_COLORS[-len(repertoires):]

    # make one plot for the line without points and errorbars
    g = sns.pointplot(x="repertoire", y=y, data=df, markers="", join=True, ci=None,
                      color=color, linestyle=linestyle, ax=ax, order=repertoires, label=label)
    # make one plot for the points without the connecting line
    g = sns.pointplot(x="repertoire", y=y, data=df,
                      palette=colors, ax=ax, order=repertoires)
    if abr_labels:
        moveset = get_moveset(repertoires[0])
        abr_dict = Constants.REPERTOIRE_ABR_DICTS[moveset]
...
def get_weight_changes(outdir: str, epoch: int = None):
    ...
    weights_before = get_model_weights(
        outdir, before_training=True, epoch=epoch)
    weights_after = get_model_weights(
        outdir, before_training=False, epoch=epoch)
...
def canoncorr(X: np.array, Y: np.array, fullReturn: bool = False) -> np.array:
    ...
    rankX = sum(np.abs(np.diagonal(T11)) > np.finfo(
        type((np.abs(T11[0, 0])))).eps*max([n, p1]))
    ...
    rankY = sum(np.abs(np.diagonal(T22)) > np.finfo(
        type((np.abs(T22[0, 0])))).eps*max([n, p2]))
    ...
    d = min(rankX, rankY)
    L, D, M = svd(Q1.T @ Q2, full_matrices=True,
...
def get_target_id(trial):
    ...
    """
    direction = (trial.target_direction + 2 *
                 np.pi) if trial.target_direction < 0 else trial.target_direction
...
def preprocess_exp_data(exp_df: pd.DataFrame, repertoire: str, subtract_mean: bool = True, remove_low_firing: bool = True):
    ...
    exp_df = pyal.combine_time_bins(exp_df, int(
        Constants.EXP_BIN_SIZE/Constants.BIN_SIZE))
    signals = [s for s in ['M1_spikes', 'PMd_spikes'] if s in exp_df.columns]
    if remove_low_firing:
        for signal in signals:
            exp_df = pyal.remove_low_firing_neurons(exp_df, signal, 5)
    for signal in signals:
        exp_df = pyal.transform_signal(exp_df, signal, 'sqrt')
    if len(signals) == 2:
        exp_df = pyal.merge_signals(exp_df, signals, "MCx_spikes")
    else:
        exp_df['MCx_spikes'] = exp_df[signals[0]]
    ...
    prep_exec_epoch = pyal.generate_epoch_fun(start_point_name='idx_go_cue',
                                              rel_start=int(
                                                  Constants.WINDOW_prep_exec[0]/Constants.EXP_BIN_SIZE),
                                              rel_end=int(
                                                  Constants.WINDOW_prep_exec[1]/Constants.EXP_BIN_SIZE)
                                              )
